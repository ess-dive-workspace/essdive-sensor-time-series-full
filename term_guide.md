...
|additional guidance|For any files following the Sensor Time Series - Full reporting format, including data, methods and attributes, and data dictionary, provide "ESS-DIVE Sensor Time Series - Full Reporting Format v1" in the `standard` field.|
...
|example|exo_12345|
|additional guidance|The sensor ID must be defined within the methods and attributes file. If more than one sensor ID is listed for a measurement column, the sensor IDs should be separated by a semicolon and space.  Changes in sensor ID during the time series can be indicated at the datetime of change in a measurement column’s corresponding `*_flag` column or in the `notes` field.|
